...
Installateur unifié + TUI GrokNight. Forge des projets Grok Build : `AGENTS.md` routeur, skills à la demande, mermaid. Linux. Interactif, **aucun flag CLI**.
...
| --- | --- | --- | --- |
| TUI / menu | `.grok/skills/forge/SKILL.md` | `forge_tui.py` | `templates/` |
| Installateur `curl \| bash` | `.grok/skills/session/SKILL.md` | `install.sh` | le TUI |
| Génération de projet | `.grok/skills/forge/SKILL.md` | `setup-grok-forge.sh`, `templates/` | — |
| Cadrage posé dans un projet | `.grok/skills/session/SKILL.md` | `templates/project/` | le TUI |
...
- Si le clone outils existe déjà : `git pull --ff-only` (pas de réinstall destructive).
...
- Si tu changes le cadrage des projets : `templates/project/` **et** `templates/apply-framing.sh`. Le TUI et `setup-grok-forge.sh` ne font qu’appeler ce script.
